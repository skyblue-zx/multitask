...
def train_model(task_type,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for step, batch in enumerate(data_loader):
            ...
            main_loss = loss_criterion(prediction, label_inputs)
            auxiliary_loss = loss_criterion(auxiliary_prediction, asr_label_inputs)
            ...
            asr_embedding_params = list(current_model.text_embeddings.parameters())

            # 计算ASR模型损失到三层FC的梯度
            auxiliary_gradient = torch.autograd.grad(
                weighted_auxiliary_loss,
                asr_embedding_params[0],
                retain_graph=True,
                create_graph=True)
