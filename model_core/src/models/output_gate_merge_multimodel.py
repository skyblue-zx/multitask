...
class OutputGateMergeMultimodel(nn.Module):
    """
    流利度二分类多模态模型
    多模态模型结果通过门限网络合并
    """
    ...
    def __init__(self, asr_pretrain_model, asr_embedding_dim, audio_embedding_dim):
        super(OutputGateMergeMultimodel, self).__init__()
        ...
        self.gate = nn.Sequential(
            nn.Linear(self.asr_embedding_dim + self.audio_embedding_dim, 2, bias=True),
            nn.Softmax(dim=1)
        )
        # self.asr_weight = nn.Parameter(torch.rand(1, 2))
        # self.main_weight = nn.Parameter(torch.rand(1, 2))
        # self.overall_bias = nn.Parameter(torch.rand(1, 2))
    ...
    def forward(self, asr_tokens, asr_segment_ids, asr_attention_mask, audio):
        ...
        # main_output = self.asr_weight * asr_output + self.main_weight * main_output + self.overall_bias
