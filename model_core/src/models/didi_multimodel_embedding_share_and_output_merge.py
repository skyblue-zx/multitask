...
class DiDiMultimodelEmbeddingShareAndOutputMerge(nn.Module):
    """
    "滴滴"多模态融合模型
    辅助多任务优化
    """
    def __init__(self, config_file_path):
        super(DiDiMultimodelEmbeddingShareAndOutputMerge, self).__init__()
        ...
        if config['embedding_path'] is None:
            ...
            # 生成Pad词对应的全零Embedding
            zero_embedding = np.zeros((1, text_embeddings.shape[1]))
            # 将全零Embedding插入Embedding列表的首位
            # 因为在Dataset中已经将词ID整体增一，将零位置让出给Pad词使用
            text_embeddings = np.concatenate([zero_embedding, text_embeddings], axis=0)
            # 根据指定的Embedding列表生成Embedding文本Embedding层
        ...
        # 文本模态子任务二分类线性层
        # Todo
        # 文本不一定是二分类，这的输出维度需要单独设置配置项
        self.text_predict_linear = nn.Linear(config['text_hidden'] * 2, config['class_num'])

        # Gate网络
        #self.gate = nn.Sequential(
        #     nn.Linear(config['text_hidden'] * 2 + config['audio_hidden'] * 2,
        #               config['text_hidden'] * 2 + config['audio_hidden'] * 2,
        #               bias=True),
        #     nn.ReLU(),
        #     nn.Linear(config['text_hidden'] * 2 + config['audio_hidden'] * 2,
        #               config['text_hidden'] * 2 + config['audio_hidden'] * 2,
        #               bias=True),
        #     nn.ReLU(),
        #     nn.Linear(config['text_hidden'] * 2 + config['audio_hidden'] * 2, 2, bias=True),
        #     nn.Softmax(dim=1)
        #)
    ...
    def forward(self, audio_inputs, audio_length, text_inputs, text_length):
        ...
        # 文本模态辅助任务 #
        ################
        text_encode_maxpool = text_encode * text_mask[:, :, None] - 10000.0 * (1 - text_mask[:, :, None])
        text_encode_maxpool, _ = torch.max(text_encode_maxpool, dim=1)  # [b,dim]
        text_logits = self.text_predict_linear(text_encode_maxpool)

        ################
        #    Gate网络   #
        ################
        #audio_encode_maxpool = audio_encode * audio_mask[:, :, None] - 10000.0 * (1 - audio_mask[:, :, None])
        #audio_encode_maxpool, _ = torch.max(audio_encode_maxpool, dim=1)  # [b,dim]
        #embedding_merge = torch.cat([text_encode_maxpool, audio_encode_maxpool], dim=1)
        #gate_output = self.gate(embedding_merge)
        ...
        merged_output = torch.cat((logits.unsqueeze(1), text_logits.unsqueeze(1)), 1)
        gate_output = nn.Softmax(dim=0)(self.gate_w)
        gated_output = merged_output * gate_output
        main_output = gated_output.sum(dim=1)
        # main_output = main_output + self.gate_b

        return main_output, text_logits
    ...
    def train_model(cls,
        ...
        for step, batch in enumerate(data_loader):
            ...
            prediction, asr_prediction = model(audio_inputs, audio_length, text_inputs, text_length)
            loss = loss_function(prediction, label_inputs)
            asr_loss = loss_function(asr_prediction, asr_label_inputs)
