...
class DiDiMultimodelEmbeddingShare(nn.Module):
    """
    "滴滴"多模态融合模型
    辅助多任务优化
    """
    def __init__(self, config_file_path):
        super(DiDiMultimodelEmbeddingShare, self).__init__()
    ...
    def forward(self, audio_inputs, audio_length, text_inputs, text_length):
        ...
        # 文本模态辅助任务 #
        ################
        text_model_output = text_encode * text_mask[:, :, None] - 10000.0 * (1 - text_mask[:, :, None])
        text_model_output, _ = torch.max(text_model_output, dim=1)  # [b,dim]
        text_logits = self.text_predict_linear(text_model_output)
